...
export default function ChattyKathy(settings) {
    ...
    function getValidatedSettings(settings) {
        if (typeof settings === 'undefined') {
            throw Error("Settings must be provided to ChattyKathy's constructor");
        }
        if (typeof settings.awsCredentials === 'undefined') {
            throw Error("A valid AWS Credentials object must be provided");
        }
        if (typeof settings.awsRegion === 'undefined' || settings.awsRegion.length < 1) {
            throw Error("A valid AWS Region must be provided");
        }
